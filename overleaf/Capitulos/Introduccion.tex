...
\item \textbf{Extension to video segmentation:} We demonstrate that our CLIP-guided prompting approach naturally extends to video by leveraging SAM2's temporal tracking capabilities. CLIP analysis on the first frame combined with SAM2's memory-based propagation enables efficient video segmentation with person-specific tracking, demonstrated on sports footage (MotoGP, NBA).

\item \textbf{Rigorous evaluation on PASCAL VOC 2012:} The system's performance is thoroughly evaluated on PASCAL VOC 2012, achieving 59.78\% mIoU in the open-vocabulary setting. The evaluation includes comprehensive metrics (mIoU, pixel accuracy, F1, precision, recall) and per-class analysis across 21 object categories.
...
\begin{itemize}
    \item Dynamic multi-scale query generation adapting to scene complexity (10-200 queries vs. 4,096 blind grid)
...
\end{itemize}
MHQR achieves +26.5\% mIoU improvement over baseline SCLIP (22.8\% $\rightarrow$ 49.3\%) on COCO-Stuff164k, surpassing supervised methods while maintaining zero-shot capability.
\end{itemize}
