...
\begin{itemize}
\item \textbf{CLIP-guided intelligent prompting for SAM2 segmentation:} The core contribution is a novel prompting strategy that extracts intelligent prompt points from SCLIP's \cite{sclip2024} high-confidence regions to guide SAM2's mask generation. This semantic-aware approach achieves \textbf{96\% reduction in prompts} compared to blind grid sampling (50-300 points versus 4,096 points) while reaching \textbf{68.09\% mIoU on PASCAL VOC 2012}, outperforming recent training-free open-vocabulary methods such as ITACLIP (67.9\%). The method assigns class labels directly from CLIP's predictions at each prompt location, avoiding the complexity of majority voting or multi-scale evaluation. This annotation-free approach enables segmentation of both discrete objects and amorphous ``stuff'' classes (sky, road, vegetation) without requiring any labeled training data.
\end{itemize}
...
\item \textbf{Extension to video segmentation via temporal tracking:} The CLIP-guided prompting approach extends to video by leveraging SAM2's temporal tracking capabilities. CLIP analysis performed on the first frame combined with SAM2's memory-based propagation enables efficient video segmentation with consistent object tracking across frames, demonstrated on sports footage (MotoGP, NBA).
...
\begin{itemize}
\item \textbf{System Architecture:} Presents a detailed overview of the system's architecture, including the integration of CLIP for language processing, SAM2 for mask generation, and the CLIP-guided intelligent prompting strategy that achieves 96\% reduction in computational overhead.
