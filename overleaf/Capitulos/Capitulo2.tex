...
Our PASCAL VOC descriptor file (\texttt{configs/cls\_voc21.txt}) provides rich descriptions for all 21 classes. For example, the background class includes terms like "sky", "wall", "tree", and "road", while the person class includes clothing variations. Full details of the descriptor file content are provided in Appendix \ref{sec:appendix_descriptors}.
...
\textbf{Implementation:} The implementation leverages PyTorch's automatic mixed precision (AMP) to accelerate inference. Code details are available in Appendix \ref{sec:appendix_optimizations}.
...
\textbf{Implementation:} The implementation applies compilation to both the CLIP vision encoder and text encoder using \texttt{mode="reduce-overhead"}. See Appendix \ref{sec:appendix_optimizations} for the code.
...
\textbf{Implementation:} The implementation processes multiple points simultaneously in batches (default size 32). See Appendix \ref{sec:appendix_optimizations} for the implementation details.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.9\textwidth]{example-image-a} % PLACEHOLDER
\caption{\textbf{[PLACEHOLDER: Intelligent Prompt Extraction Process]} This figure should visualize the 5-stage pipeline: (1) SCLIP confidence map, (2) Binary thresholding, (3) Connected component clustering (showing distinct colors for regions), (4) Size filtering (removing small noise), and (5) Final centroid computation (showing points on the original image). This visual step-by-step breakdown is crucial for explaining the core contribution.}
...
\caption{Comparison between SCLIP dense prediction baseline and the proposed CLIP-guided prompting approach. Three representative examples demonstrate key advantages: superior boundary quality through SAM2 integration (+13.19pp on horse), improved small object detection (+7.77pp on bottle), and cleaner instance separation in cluttered scenes (+7.00pp on person). Red circles highlight baseline errors; green checkmarks indicate improvements. The proposed method achieves 96\% prompt reduction (3-15 points vs 4096 grid) while improving accuracy across all scenarios.}
...
\fbox{\textbf{Thesis Contribution}} This enhancement improves boundary quality in the final segmentation.

While SAM2 provides high-quality instance boundaries, the final merged segmentation can exhibit minor boundary artifacts at class transitions, particularly in regions where multiple SAM2 masks overlap or where SCLIP predictions have low confidence. To address this, the best-performing configuration (68.09\% mIoU on PASCAL VOC 2012) incorporates \textbf{Dense Conditional Random Field (CRF)} post-processing \cite{krahenbuhl2011efficient} as a refinement stage.
...
\begin{itemize}
    \item \textbf{mIoU improvement:} +1-2 percentage points (66.5\% → 68.09\% on PASCAL VOC 2012)
...
\textbf{Note:} While computationally more expensive, this refinement is essential for achieving state-of-the-art results and is considered an integral part of the high-accuracy pipeline configuration.
...
Once all prompts are registered on frame 0, SAM 2 propagates masks across the entire video using its memory-based tracking mechanism. This process involves frame encoding, memory retrieval using cross-attention, mask prediction, and memory updates. 

For a detailed mathematical explanation of the SAM 2 memory mechanism and attention operations, please refer to \textbf{Appendix \ref{sec:appendix_sam2}}.

The propagation iterates through all video frames, applying this process sequentially to obtain segmentation masks for all tracked objects.
...
\caption{Video segmentation and inpainting pipeline demonstration. The figure shows the complete workflow from original video frames to final inpainted results, organized chronologically from top to bottom. (Top rows) Original video frames sampled at regular intervals, showing temporal evolution of the scene with camera motion and object dynamics. (Middle rows) CLIP-guided SAM2 segmentation results overlaid on frames, demonstrating consistent object tracking across time through SAM2's memory-based propagation. Colored masks indicate different semantic classes identified from first-frame CLIP analysis. (Bottom rows) VACE video inpainting results showing realistic object removal with temporal consistency. The system maintains coherent texture synthesis across frames, preserving motion blur, lighting changes, and perspective transformations. Key observations: (1) First-frame CLIP analysis (50-300 prompts) provides semantic initialization. (2) SAM2 memory mechanism propagates masks across all frames without per-frame CLIP inference. (3) VACE diffusion model generates temporally coherent inpainting, avoiding flickering artifacts common in frame-by-frame approaches. (4) Processing achieves 3-8 minutes for 81 frames at 480p on consumer GPU (GTX 1060 6GB). Example applications include sports footage editing (removing advertisements, isolating players), surveillance video redaction (anonymizing individuals), and creative video manipulation (background replacement, object insertion). The pipeline demonstrates practical video editing capabilities driven entirely by natural language prompts, requiring no manual annotation or frame-by-frame interaction.}
...
\subsection{Video Inpainting with WAN 2.1 VACE}
\label{sec:vace_integration}

While the CLIP-guided video segmentation pipeline (Section \ref{sec:clip_guided_approach}) generates high-quality segmentation masks across video frames, practical video editing applications require the ability to modify or remove detected objects. This section describes the integration of WAN 2.1 VACE \cite{vace2025} for text-driven video inpainting, enabling complete video editing workflows from segmentation to content generation.
...
\subsubsection{Mask Video Generation}
...
\paragraph{Binary Mask Extraction}
...
The binary mask frames are saved as PNG images and encoded into an MP4 video using FFmpeg with H.264 codec. The specific encoding command ensures compatibility with VACE's video input requirements (see Appendix \ref{sec:appendix_vace}).

This ensures compatibility with VACE's video input requirements while maintaining lossless mask boundaries through appropriate codec settings (CRF 23 for perceptually lossless quality).

\paragraph{Dimension Handling}
...
\subsubsection{Preview Video Generation}
...
\subsubsection{VACE Inference Configuration}

WAN 2.1 VACE is invoked with the following configuration optimized for consumer hardware (6GB VRAM):

\paragraph{Model Selection}
The implementation uses \texttt{VACE-1.3B} (1.3 billion parameters) rather than \texttt{VACE-14B} due to memory constraints. While VACE-14B provides finer detail synthesis, VACE-1.3B produces acceptable results within hardware limitations.
...
VACE inference is invoked through the official Python API with directory handling to accommodate relative paths in the VACE codebase:

\paragraph{VACE Inference Arguments}

The VACE inference is invoked with specific arguments to handle path management and model configuration. The implementation details, including the Python code for argument preparation, are provided in Appendix \ref{sec:appendix_vace}.

This careful path management ensures VACE can locate its configuration files and model checkpoints while maintaining clean integration with the broader pipeline.
...
This trades disk I/O for reduced VRAM usage, critical for processing 81-frame videos on 6GB GPUs.

\paragraph{Model Offloading}
VACE supports optional CPU offloading for model components not actively used during inference. While this increases latency, it enables processing on memory-constrained hardware.

\paragraph{Resolution Reduction}
The implementation defaults to 480p resolution. While 720p is supported by VACE, it requires approximately 10-12GB VRAM, exceeding consumer hardware capabilities. For applications requiring higher resolution, users can run inference at 480p and apply video super-resolution as post-processing.

\subsubsection{Output and Cleanup}
...
\begin{itemize}
    \item \textbf{Length restriction:} 81-frame maximum due to memory constraints
    \item \textbf{Resolution limitation:} 480p for 6GB VRAM, 720p requires 12GB+
...
Future work (Chapter 5) discusses potential solutions, including sliding window processing for longer videos and instance-level mask filtering.

\subsubsection{Integration with CLIP-Guided Segmentation}
