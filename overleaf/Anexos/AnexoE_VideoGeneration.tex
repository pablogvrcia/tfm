...
SAM2's video predictor is initialized with the Hiera-Large architecture. Video frames and intermediate states are offloaded to CPU RAM to conserve GPU memory on consumer hardware (6GB VRAM). Each extracted prompt point is registered as a foreground point on frame 0, associating spatial locations with object classes for temporal tracking.
...
Video processing on consumer GPUs (GTX 1060 6GB) requires careful memory management. The implementation employs CPU offloading for video frames and intermediate states, temporary file storage for rendered frames, and single-pass processing to minimize overhead. This enables video segmentation within 6GB VRAM constraints, though with increased CPU-GPU transfer overhead.
...
\subsubsection{Model Offloading}
